...
class Bot:
  ...
  def post_flop(self):
          ...
          case 1:
            if self.flush_cards_count(self.getAllCards()) == 4: #flush
              if self.straight_case(self.getAllCards()) != 0: #straight
                if self.obs.get_call_size() > 6*self.obs.big_blind: #calls
                  return 0
                else:
                  return self.try_raise_bb(4)
              else:
                if self.highCardRank(handHighCard) == 4:
                  ...
                  if self.obs.get_call_size() > 3*self.obs.big_blind: ##calls
                    return 0
                  else:
                    return self.try_raise_bb(4)
                else:
              ...
              if self.straight_case(self.getAllCards()) != 0: #straight
                if self.highCardRank(handHighCard) == 4:
                  ...
                  if self.obs.get_call_size() > 2 * self.obs.big_blind:  ##calls
                    return 0
                  else:
                    return self.try_raise_bb(4)
                else:
              ...
              if self.highCardRank(self.findPairValue()) < 3: #par is highest card
                if self.flush_cards_count(self.getAllCards()) == 4: #flush
                  ...
                  if self.straight_case(self.getAllCards()) != 0: #straight
                    ...
                    if self.obs.get_call_size() > 0.5 * self.obs.get_pot_size():  ##calls
                      return 0
                    else:
                      return self.try_raise_pot(0.5)
              else:
                if self.flush_cards_count(self.getAllCards()) == 4: #flush
                  if self.straight_case(self.getAllCards()) != 0: #straight
                    ...
                    if self.obs.get_call_size() > 5 * self.obs.big_blind:  ##calls
                      return 0
                    else:
                      return self.try_raise_bb(4)
                else:
    ...
    print(f"FLUSH count full: {self.flush_cards_count(allCards)}")
    print(self.straight_case(allCards))
